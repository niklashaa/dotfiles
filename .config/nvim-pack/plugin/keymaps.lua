...
-- Reload config without restarting: clear the config.* module cache (require()
-- caches modules, so edits to lua/config/*.lua wouldn't otherwise apply) then
-- re-source init.lua. Note: re-runs plugin setup()/keymaps; autocmds use cleared
-- augroups so they don't stack. For a fully clean slate, restart instead.
